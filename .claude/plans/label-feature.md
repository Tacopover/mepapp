...
**Phase 1 — Done** 2026-09-26, commit `58bb6b6`. `core/src/stamp-properties.ts` (key catalogue, `buildStampPropertyContext`, `resolveStampProperty` / `resolveStampPropertyValue` with an `inherited` flag, device and cable formatting); `GlobalPropertyDefs` moved to `core/src/custom-properties.ts` (the dialog re-exports it). Properties panel: `TerminalCircuitSection.tsx` shows read-only rows under the picker (Circuit number, Circuit prefix, Circuit name, Panel, Circuit type, Phase, Device, Cable, circuit custom properties), "(panel default)" on an inherited value, "Varies" across a multi-selection; a panel's equipment stamp shows a read-only Panel name row. Verified: `pnpm build` (9 of 9 tasks), `pnpm turbo run test` (259 core, 13 ui, 13 pdf-engine-mupdf passed; 12 new core cases). Real Playwright run with real canvas clicks, 12 of 12 checks passed, no page errors: panel name row, every circuit row with the inherited marks, live update after a panel rename while selected, no rows for a terminal outside a circuit, "Varies" in a multi-selection, common value once both terminals share a circuit. Setup of the panel and circuit used the scene API, not the circuit UI. Not done: an empty value shows an empty row (for example Circuit name when the circuit has none), not a hidden row.

**Phase 2 — Done** 2026-09-26, commit `202ce2c`. `core/src/stamp-label.ts` (`StampLabel`, `StampLabelLayouts`, `DEFAULT_STAMP_LABEL_STYLE`, `computeStampLabelPlacement` with the side justification, `resolveStampLabelText`); schema 10 → 11 adds `stampLabelLayouts` (default `{}`, `requireRecord` validator). Render: `render/src/stampLabels.ts` (`syncStampLabels` diffs against a per-document node cache keyed `stampId:labelId`; text laid out at 4× and scaled down so it stays sharp when zoomed; box redrawn only when text or style changes); `SketchDocument.labelLayer`, `labelNodes`, `stampLabelLayouts`; `SketchScene.setLabelContext`, `getStampLabelLayouts`, `setStampLabelLayout`, private `syncLabels`, called from `syncDrawingLayer`, `notifyCircuitsChanged`, `setStampProperty`, `applyCustomPropertyCascade`, both capacity setters, `updateCustomStampDefinition` and `activateInternal`; export and load carry the layouts. `App.tsx` pushes the custom property definitions and the language to the scene. Verified: `pnpm build` (9 of 9 tasks), `pnpm turbo run test` (265 core, 13 ui, 13 pdf-engine-mupdf passed; 6 new core cases incl. the v10 migration). Real Playwright run, 14 of 14 checks passed, no page errors: circuit label only on the terminal in a circuit, prefix and suffix, panel name label, text follows a panel-default prefix change and a panel rename, label follows a real mouse drag, a real +90° rotate moves the label to the other side, rail Undo moves it back, the real language toggle switches the name label to Dutch, Delete key removes the labels and Undo restores them, layouts and all 4 labels come back after Menu > Save and reopen. Layouts were set through `setStampLabelLayout` (no editor until Phase 3). Screenshots checked. Not done: performance not measured on a large document (G1).

**Phase 3 — Done** 2026-09-26, commit `bf465e5`. **Deviation from §7:** the Element Editor only edits custom definitions (a library stamp opens it only in "duplicate" mode, which makes a new custom copy), so the Labels tab alone cannot give a library stamp labels. The editor is one component, `StampLabelsEditor.tsx`, used in two places: (1) a new "Edit labels…" button on every placed stamp with a definition (Properties panel) opens `StampLabelsDialog.tsx`, for library and custom definitions, with the stamp's real values in the preview; (2) the Element Editor's Labels tab (enabled, create and edit mode), saved through `onSave`'s new second argument. The Element Editor's drawing grid stays mounted (hidden) on the Labels tab because the shape editor keeps a ref into it. Editor: click to add a label, drag the anchor dot to move it, Delete removes, own Undo/Redo (buttons and Ctrl+Z / Ctrl+Y), grouped property picker (Stamp / Custom / Circuit / Panel), prefix, suffix, font size, text color, background and border toggles with color; sample values ("L1.3", "Panel A", …) in italics where the stamp has no value yet; a label whose key is gone shows "missing: <key>" in red. Its keys are caught in the window capture phase with `stopPropagation`, so Delete in the dialog does not reach the canvas (the first run deleted the canvas stamp and closed the dialog) or the hidden shape editor. Verified: `pnpm build` (9 of 9 tasks). Real Playwright run with real clicks, typing and drags, 15 of 15 checks passed, no page errors: dialog opens, two clicks add two labels, property and suffix and prefix change the preview, sample circuit value while the stamp has no circuit, anchor drag moves the label without adding one, Delete removes one and does not delete the canvas stamp, Undo restores it, Save draws the label on the canvas, picking "New circuit…" in the real Circuit picker makes the circuit label appear, reopening shows the saved layout with the real circuit value, and the Element Editor Labels tab on a new element (with a drawn rectangle) saves its layout on Create. Screenshots checked after two layout fixes (hint above the editor; larger preview that draws outside its box). Not done: an existing Element Editor bug seen while testing — with a canvas stamp selected, Delete inside the Shapes tab also reaches the canvas (`useShapeDrawEditor` does not stop propagation); not fixed here (scope). Undo history of the Labels tab resets when you switch tabs.

**Phase 4 — Done** 2026-09-26, commit `PHASE4_HASH`. Core: `StampLabelVisibility`, `DEFAULT_STAMP_LABEL_VISIBILITY` (on), `isStampLabelVisible`, `stampPropertyGroupOf`. Scene: `setLabelVisibility` (filters inside `syncLabels`), new `stampLabelLayoutsChanged` event; `useSketchScene` keeps `stampLabelLayouts`. UI: `LabelVisibilityControl.tsx` in the status bar — a "Labels" checkbox chip plus a ▴ drop-up with Kinds (Stamp, Custom, Circuit, Panel) and one entry per placed definition that has a layout, grouped under Terminals / Equipment / Fittings with a mixed-state group checkbox; closes on an outside click. The setting is stored per installation in localStorage `mepapp.settings.labelVisibility.v1`. Circuits mode does not change it (D4). Verified: `pnpm build` (9 of 9 tasks), `pnpm turbo run test` (267 core, 13 ui, 13 pdf-engine-mupdf passed; 2 new core cases). Real Playwright run, 11 of 11 checks passed, no page errors: on by default, toggle off and on, drop-up lists both definitions, hiding the Circuit kind hides only the circuit label, hiding a definition hides its labels, the group checkbox follows and re-shows it, outside click closes, the setting is in localStorage and survives a reload. Screenshot checked.
