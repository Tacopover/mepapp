...
const ROWS_TEMPLATE: SchematicTemplate = (() => {
  ...
  return {
    ...
    layoutBlocks: [
      ...
      block('bus', 'busbar', 30, 42, { width: 2, height: 430 }),
      block('section', 'section', 34, 40, { width: 190 }),
...
const COLUMNS_TEMPLATE: SchematicTemplate = (() => {
  ...
  const standardBlocks: SchematicBlock[] = [
    ...
    block('count', 'countCell', 0, 116, { width: 14, binding: '{count(terminals)}' }),
    block('derived', 'derivedCell', 0, 124, { width: 14, binding: '{circuit.diversifiedCapacity}' }),
  ];
  const groups: CircuitGroupDefinition[] = [
  ...
  return {
    ...
    layoutBlocks: [
      ...
      block('feed', 'feedCable', 12, 14),
      block('main', 'mainDevice', 14, 26),
      block('bus', 'busbar', 30, 42, { width: 720, height: 2 }),
      block('section', 'section', 30, 44, { height: 140 }),
      block('totals', 'totalsTable', 12, 546, { tableRows: TOTALS_ROWS }),
    ],
