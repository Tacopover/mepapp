...
import { resolveAlignmentSnap, type AlignmentGuide } from './alignmentGuides.js';
...
export interface SnappedPoint {
  point: Vec2;
  /** Alignment guide line(s) to render for the current frame — only populated for 'move-selection'; every other kind gets an empty array. */
  guides: AlignmentGuide[];
}

/**
 * Adjusts a raw world-space point derived from the pointer, before a tool applies it to
 * geometry — the shared seam for snap-to-object alignment guides (move-selection) and
 * angle-snap-while-drawing-segments (draw-segment). Every other call site gets back the
 * point unchanged and an empty guides list.
 */
export function resolveSnappedPoint(rawWorldPoint: Vec2, context: DragSnapContext): SnappedPoint {
  ...
  if (context.kind === 'move-selection') {
    ...
    return resolveAlignmentSnap(context.ctx, state, rawWorldPoint, draggedBounds, excludeIds);
  }
